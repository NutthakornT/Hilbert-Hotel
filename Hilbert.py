...
class Hash:
    ...
    def insert(self, data):
        load_factor = (self.size + 1) / len(self.table) * 100
    ...
    def rehash(self):
        ...
        # ใส่ของเก่ากลับ โดยไม่เรียก rehash อีก
...
while True:
    ...
    if choice == 0:
        ...
        # r_last = ชั้นสูงสุดที่ใช้แล้ว (เช่น max route ใน route_list)

        r_new = len(routes) + 1
        for idx, guest in enumerate(old_guests, start=1):
            p = idx  # ใช้ลำดับที่ loop เจอ
            guest.room = (2**r_new) * (2 * p - 1) - 1
